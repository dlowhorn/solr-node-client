...
/**
 * Filter the set of documents found before to return the result with the given `field` and `values`
 *
 * @param {String} field - name of field
 * @param {String|Array} values - list of values the field must be in
 *
 * @return {Query}
 * @api public
 *
 * @example
 * var query = client.createQuery();
 * query.q({ '*' : '*' }).inFilter('id', '100,101,102')
 * // also works
 * query.q({ '*' : '*' }).inFilter('id', [100,101,102])
 */

Query.prototype.inFilter = function(field,values){
   var self = this;
   value = format.dateISOify(value);
   var parameter = 'fq=';
   parameter += field + ':(' + encodeURIComponent(values) + ')';
   this.parameters.push(parameter);
   return self;
}
